...
// @LOAD materials/textures/wall_bump.bmp
...
void main() {
	...
	float scaleTexCoord = 2;
	vec3 normalTex = texture(texNormal,t / scaleTexCoord).rgb * 2.0 - 1.0;
	...
	float intensity = 0;
	while(size > 16) {
		intensity += (noise3D(oPosition.x / size, oPosition.y / size,oPosition.z/ size) * size);
		size = size / 2.0;
	}
	vec3 genTexel = vec3(intensity,intensity,intensity) / 32;
	vec4 turbulence = vec4(genTexel,1.0);
	*/
